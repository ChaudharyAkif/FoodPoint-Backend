...
export const updateProduct = async (req: Request, res: Response) => {
  try {
    ...
    if (Array.isArray(updateData.optionGroups)) {
      updateData.optionGroups = updateData.optionGroups.map((g: any) =>
        typeof g === 'object' && g._id ? g._id : g
      );
    }

    if (Array.isArray(updateData.extras)) {
      updateData.extras = updateData.extras.map((e: any) =>
        typeof e === 'object' && e.name ? e.name : String(e)
      );
    }
    ...
    // ✅ Two-way sync
    const oldGroups = (oldProduct.optionGroups || []).map((id: any) => id.toString());
    ...
    const removedGroups = oldGroups.filter((id: string) => !newGroups.includes(id));
    const addedGroups = newGroups.filter((id: string) => !oldGroups.includes(id));
...
console.log(req.body)
    ...
    // Sync option groups
    await Promise.all(
      newSavedProducts.map((product) => {
        if (product.optionGroups?.length) {
          return OptionGroup.updateMany(
            { _id: { $in: product.optionGroups } },
            { $addToSet: { linkedProducts: product._id } }
          );
        }
