...
export const register = async (req, res) => {
    try {
        ...
        if (existingUser)
            return res.sendMessage(400, 'User already exists');
...
export const login = async (req, res) => {
    try {
        ...
        if (!user)
            return res.sendMessage(400, 'User not found');
        const isMatch = await bcrypt.compare(password, user.password);
        if (!isMatch)
            return res.sendMessage(400, 'Wrong password');
...
export const createMultipleCashiers = async (req, res) => {
    try {
...
export const changePassword = async (req, res) => {
    try {
        ...
        if (!req.user?.id)
            return res.sendMessage(401, 'Unauthorized');
        const user = await User.findById(req.user.id);
        if (!user)
            return res.sendMessage(404, 'User not found');
        const isMatch = await bcrypt.compare(oldPassword, user.password);
        if (!isMatch)
            return res.sendMessage(400, 'Old password is incorrect');
...
export const forgotPassword = async (req, res) => {
    ...
    try {
        ...
        if (!email)
            return res.sendMessage(400, 'Email is required');
        const user = await User.findOne({ email });
        if (!user)
            return res.sendMessage(405, 'User not found');
...
export const resetPassword = async (req, res) => {
    try {
        ...
        if (!newPassword)
            return res.sendMessage(400, 'New password required');
...
export const checkRole = async (req, res) => {
    try {
        const { email } = req.body;
        if (!email)
            return res.sendMessage(400, 'Email is requireds');
        const user = await User.findOne({ email });
        if (!user)
            return res.sendMessage(404, 'User not found');
...
export const getAllCashiers = async (_req, res) => {
    try {
...
export const deleteCashier = async (req, res) => {
    try {
        ...
        if (!deleted)
            return res.sendMessage(404, 'Cashier not found');
