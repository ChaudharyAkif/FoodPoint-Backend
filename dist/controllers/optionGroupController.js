...
export const createGroup = async (req, res) => {
    try {
        ...
        if (linkedProducts && linkedProducts.length > 0) {
            await Product.updateMany({ _id: { $in: linkedProducts } }, { $addToSet: { optionGroups: savedGroup._id } });
        }
...
export const updateGroup = async (req, res) => {
    try {
        ...
        await OptionGroup.findByIdAndUpdate(id, {
            name,
            isRequired: Boolean(isRequired),
            options: options || [],
            linkedProducts: linkedProducts || []
        ...
        const formatted = {
            ...finalGroup,
            linkedProductNames: finalGroup?.linkedProducts?.map((p) => p.productName) || []
        };
        res.json(formatted);
    }
    catch (error) {
