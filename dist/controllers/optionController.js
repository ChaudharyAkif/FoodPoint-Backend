    ...
    try {
        ...
        const formatted = options.map((opt) => ({
            ...
            linkedProductNames: opt.linkedProducts
                ? opt.linkedProducts.filter((p) => p !== null).map((p) => p.productName)
                : []
        }));
        res.json(formatted);
    }
...
export const updateOption = async (req, res) => {
    try {
        ...
        const updatedOption = await Option.findByIdAndUpdate(id, { name, price: Number(price) || 0, linkedProducts: linkedProducts || [] }, { new: true }).populate('linkedProducts', 'productName').lean();
        // FIXED: Guard against null result
        if (!updatedOption)
            return res.status(404).json({ message: 'Option update failed' });
        ...
        const formatted = {
            ...updatedOption,
            linkedProductNames: updatedOption.linkedProducts?.map((p) => p.productName) || []
        };
        res.json(formatted);
    }
    catch (error) {
